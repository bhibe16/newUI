...
<body class="main-content min-h-screen font-sans bg-gray-50" x-data="successionPlanning()" x-cloak>
                        ...
                        <div class="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300">
                            ...
                            <div class="p-6">
                                <div class="space-y-3">
                                    ...
                                    <!-- Mentor Assignment -->
                                    <div>
                                        <p class="text-sm text-gray-500">Assigned Mentor</p>
                                        <template x-if="employee.mentor">
                                            <div class="mt-1 p-2 bg-blue-50 rounded-lg">
                                                <p class="font-medium" x-text="employee.mentor.name"></p>
                                                <p class="text-xs text-gray-600" x-text="employee.mentor.position"></p>
                                            </div>
                                        </template>
                                        <template x-if="!employee.mentor">
                                            <p class="text-gray-500 text-sm mt-1">No mentor assigned</p>
                                        </template>
                                    </div>
                ...
                <div class="p-6 space-y-6 overflow-y-auto" style="max-height: calc(90vh - 120px)">
                    ...
                    <div class="pt-4 border-t border-gray-200">
                        <div class="flex justify-between items-center mb-4">
                            <h3 class="text-lg font-semibold text-gray-800">Mentor Assignment</h3>
                            <button @click="showMentorAssignmentModal = true" class="px-3 py-1 bg-blue-600 text-white rounded-md text-sm hover:bg-blue-700 transition flex items-center">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" viewBox="0 0 20 20" fill="currentColor">
                                    <path d="M8 9a3 3 0 100-6 3 3 0 000 6zM8 11a6 6 0 016 6H2a6 6 0 016-6z" />
                                </svg>
                                <span x-text="selectedEmployee && selectedEmployee.mentor ? 'Change Mentor' : 'Assign Mentor'"></span>
                            </button>
                        </div>
                        
                        <template x-if="selectedEmployee && selectedEmployee.mentor">
                            <div class="p-4 bg-blue-50 rounded-lg">
                                <div class="flex items-center gap-4">
                                    <div class="bg-blue-100 text-blue-800 rounded-full w-10 h-10 flex items-center justify-center font-semibold">
                                        <span x-text="selectedEmployee.mentor.name.split(' ').map(n => n[0]).join('')"></span>
                                    </div>
                                    <div>
                                        <p class="font-medium" x-text="selectedEmployee.mentor.name"></p>
                                        <p class="text-sm text-gray-600" x-text="selectedEmployee.mentor.position"></p>
                                        <p class="text-xs text-blue-500" x-text="selectedEmployee.mentor.department"></p>
                                    </div>
                                </div>
                            </div>
                        </template>
    ...
    <div x-show="showMentorAssignmentModal" class="fixed inset-0 z-50 overflow-y-auto">
        <div class="modal-overlay absolute w-full h-full bg-gray-900 bg-opacity-50" @click="showMentorAssignmentModal = false"></div>
        <div class="relative min-h-screen flex items-center justify-center p-4">
            <div class="modal-content bg-white rounded-xl shadow-xl w-full max-w-md">
                ...
                <div class="bg-white px-6 py-4 flex justify-between items-center border-b border-gray-200">
                    ...
                    <button @click="showMentorAssignmentModal = false" class="text-gray-700 hover:text-gray-900">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                        </svg>
                    </button>
                ...
                <div class="p-6">
                    <div class="mb-4">
                        ...
                        <div class="bg-gray-50 p-3 rounded-lg">
                            <p class="font-medium" x-text="selectedEmployee ? selectedEmployee.name : ''"></p>
                            <p class="text-sm text-gray-500" x-text="selectedEmployee ? selectedEmployee.position : ''"></p>
                        </div>
                    </div>
                    
                    <div class="mb-4">
                        <label class="block text-sm font-medium text-gray-700 mb-1">Select Mentor</label>
                        <select x-model="selectedMentor" class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500">
                            <option value="">Select a mentor</option>
                            <template x-for="mentor in mentors" :key="mentor.id">
                                <option :value="mentor.id" x-text="mentor.name + ' (' + mentor.position + ')'"></option>
                            </template>
                        </select>
                    </div>
                </div>
                ...
                <div class="bg-gray-50 px-6 py-4 flex justify-end border-t border-gray-200">
                    <button @click="showMentorAssignmentModal = false" class="px-4 py-2 border border-gray-300 rounded-md text-gray-700 hover:bg-gray-100 transition mr-3">
                        Cancel
                    </button>
                    <button @click="assignSelectedMentor()" class="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition">
                        <span x-text="selectedEmployee && selectedEmployee.mentor ? 'Change Mentor' : 'Assign Mentor'"></span>
                    </button>
                </div>
            </div>
    ...
    <div x-show="showSuccessNotification" 
        ...
        class="fixed bottom-4 right-4 bg-green-500 text-white px-4 py-2 rounded-md shadow-lg flex items-center">
    ...
    function successionPlanning() {
        return {
            ...
            showMentorAssignmentModal: false,
            selectedMentor: localStorage.getItem('selectedMentor') || null, // Fetch saved mentor from localStorage
            ...
            mentors: [
                {
                    id: 'mentor1',
                    name: 'Catherine G. Saminan',
                    position: 'Purchasing Manager',
                    department: 'Operations'
                },
                {
                    id: 'mentor2',
                    name: 'Christian B. Noora',
            ...
            assignSelectedMentor() {
                if (!this.selectedEmployee || !this.selectedMentor) return;
                
                const mentor = this.mentors.find(m => m.id === this.selectedMentor);
                if (!mentor) return;
                
                // Update the selected employee's mentor
                this.selectedEmployee.mentor = mentor;
                
                // Find and update in the employees array
                const index = this.employees.findIndex(e => e.id === this.selectedEmployee.id);
                if (index !== -1) {
                    this.employees[index].mentor = mentor;
                }
                
                // Show success notification
                this.successMessage = `Assigned ${mentor.name} as mentor to ${this.selectedEmployee.name}`;
                ...
                this.showMentorAssignmentModal = false;
                this.selectedMentor = null;
                
                // Save the selected mentor to localStorage
                localStorage.setItem('selectedMentor', this.selectedMentor);
            }
        }
    }
</script>
